...
func update_info_from_level(level : LevelOrb):
	if level != null:
		...
		all_bugs.button_pressed = level.level_all_bug_pieces
		all_bugs.disabled = level.level_all_bug_pieces
		
		bench.visible = level.b_time != 0
		bench.button_pressed = level.level_otd_met
		bench.disabled = level.level_otd_met
		#todo: time formatting
		bench.text = "OTD (" + str(level.b_time) + ")"
		score.visible = level.level_score_bench != 0
		score.button_pressed = level.level_score_bench_met
		score.disabled = level.level_score_bench_met
		#todo: formatting
		score.text = "Score (" + str(level.level_score_bench) + ")"
		
		clipboard_button.button_pressed = false
	else:
...
func _on_Node2D_set_toggle_ui_switch(node_name, to):
	match(node_name):
		"CompletedCheck":
			$VBoxContainer/Checks/CompletedCheck.button_pressed = to
			return
		"AllJems":
			$VBoxContainer/Checks/AllJems.button_pressed = to
			return
		"FoundGR17":
			$VBoxContainer/Checks/FoundGR17.button_pressed = to
			return
		"AllBugs":
			$VBoxContainer/Checks/AllBugs.button_pressed = to
			return
		"Benchmark":
			$VBoxContainer/Checks/Benchmark.button_pressed = to
			return
...
func _on_TextureButton_toggled(button_pressed):
	if button_pressed:
		if SpaceShip.current_orb != null:
			DisplayServer.clipboard_set(SpaceShip.current_orb.levelID) 
	else:
		if SpaceShip.current_orb != null:
			DisplayServer.clipboard_set(SpaceShip.current_orb.levelID) 
			clipboard_button.button_pressed = true
